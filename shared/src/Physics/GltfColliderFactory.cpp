...
namespace Details {
    ...
    void loadGltfFile(tinygltf::Model& model, std::filesystem::path path, std::filesystem::path parentDir) {
        ...
        if (!fileBuf) {
            //moe::Logger::error("Failed to load glTF file: {}", path.string());
        ...
        if (!warn.empty()) {
            //moe::Logger::warn("GLTF loader warning: {}", warn);
        }

        if (!success) {
            //moe::Logger::error("Failed to load glTF: {}", err);
    ...
    Span<const T> getPackedBufferSpan(
        ...
        if (bs != sizeof(T)) {
            //moe::Logger::error("Accessor buffer is not tightly packed, expected {}, got {}", sizeof(T), bs);
    ...
    glm::mat4 loadTransform(const tinygltf::Node& node) {
        ...
        if (!node.matrix.empty()) {
            //moe::Logger::warn("Matrix not supported for ColliderFactory");
            return transform;
        }
        ...
        for (size_t pi = 0; pi < mesh.primitives.size(); ++pi) {
            ...
            if (triangleIndices.empty()) {
                ...
                if ((positions.size() % 3) != 0) {
                    //moe::Logger::error("No indices and POSITION count is not multiple of 3");
            ...
            if (triangleIndices.size() % 3 != 0) {
                //moe::Logger::error("Triangle indices count is not a multiple of 3");
